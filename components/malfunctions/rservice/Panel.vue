<template>
  <!-- eslint-disable no-irregular-whitespace -->
  <div class="row">
    <div class="col-12">
      <el-button
        @click="openModalCreate"
        type="success"
        size="medium"
        class="mb-15"
      >
        Создать коллекцию неисправностей
      </el-button>
    </div>
    <!--  -->
    <div
      v-for="(collection, index) in malfunctions"
      :key="index"
      class="col-6 col-lg-4 mb-30"
    >
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="5">{{ collection.name }}</td>
            <td>
              <el-button
                @click="openModalUpdate(collection)"
                title="Редактиоравть коллекцию"
                type="success"
                size="mini"
                icon="el-icon-edit"
              />

              <el-popconfirm
                @onConfirm="removeMalfuntion(collection._id)"
                title="Удалить категорию?"
                confirm-button-text="Да"
                confirm-button-type="success"
                cancel-button-type="default"
                cancel-button-text="Нет, спасибо"
              >
                <el-button
                  slot="reference"
                  :loading="loading"
                  title="Удалить коллекцию"
                  size="mini"
                  type="danger"
                  icon="el-icon-delete"
                />
              </el-popconfirm>
            </td>
          </tr>
          <tr v-for="(malfunction, idx) in collection.malfunctions" :key="idx">
            <td
              colspan="4"
              style="text-align: left; padding-left: 10px; min-width: 290px"
            >
              {{ malfunction.name }}
            </td>
            <td>от {{ malfunction.price }} р.</td>
            <td>от {{ malfunction.time || 30 }} мин.</td>
          </tr>
        </tbody>
      </table>
    </div>
    <!--  -->
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">ноутбуки</td>
          </tr>
          <tr>
            <td>Чистка от пыли</td>
            <td>Чистка от воды</td>
            <td>Сильно греется</td>
            <td>Не заряжается</td>
            <td>Не включается</td>
            <td>Выключается</td>
          </tr>
          <tr>
            <td>от 800</td>
            <td>от 1250</td>
            <td>от 400</td>
            <td>от 800</td>
            <td>от 400</td>
            <td>от 800</td>
          </tr>
          <tr>
            <td>Разбит экран</td>
            <td>Нет изображения</td>
            <td>Отсутствует звук</td>
            <td>Сломан разъём</td>
            <td>Глючит / Зависает</td>
            <td>Проблемы с ОС</td>
          </tr>
          <tr>
            <td>от 600</td>
            <td>от 450</td>
            <td>от 500</td>
            <td>от 600</td>
            <td>от 350</td>
            <td>от 500</td>
          </tr>
          <tr>
            <td>Не работает клавиатура</td>
            <td>Не работает тачпад</td>
            <td>Не работает камера / микрофон</td>
            <td>Не работает wi-fi / bluetooth</td>
            <td>Не видит устройства</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 450</td>
            <td>от 450</td>
            <td>от 350</td>
            <td>от 500</td>
            <td>от 350</td>
            <td>от 350</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">проекторы</td>
          </tr>
          <tr>
            <td>Замена матрицы</td>
            <td>Замена диода</td>
            <td>Замена лампы</td>
            <td>Замена фильтра</td>
            <td>Юстировка линзы</td>
            <td>Прошивка ПО</td>
          </tr>
          <tr>
            <td>от 750</td>
            <td>от 650</td>
            <td>от 850</td>
            <td>от 450</td>
            <td>от 800</td>
            <td>от 800</td>
          </tr>
          <tr>
            <td>Чистка системы охлаждения</td>
            <td>Чистка оптической системы</td>
            <td>Ремонт системной платы</td>
            <td>Ремонт блока управления</td>
            <td>Ремонт блока питания</td>
            <td>Ремонт системы охлаждения</td>
          </tr>
          <tr>
            <td>от 850</td>
            <td>от 750</td>
            <td>от 950</td>
            <td>от 850</td>
            <td>от 750</td>
            <td>от 950</td>
          </tr>
          <tr>
            <td>Замена главной линзы</td>
            <td>Замена поляризатора</td>
            <td>Замена балластера</td>
            <td>Замена колеса DLP</td>
            <td>Замена блока питания</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 650</td>
            <td>от 750</td>
            <td>от 950</td>
            <td>от 550</td>
            <td>от 800</td>
            <td>от 450</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">мониторы</td>
          </tr>
          <tr>
            <td>Замена матрицы</td>
            <td>Ремонт подсветки</td>
            <td>Ремонт инвертора</td>
            <td>Замена процесора</td>
            <td>Замена разъёма</td>
            <td>Прошивка ПО</td>
          </tr>
          <tr>
            <td>от 1500</td>
            <td>от 1600</td>
            <td>от 1200</td>
            <td>от 1400</td>
            <td>от 1400</td>
            <td>от 900</td>
          </tr>
          <tr>
            <td>Ремонт платы цветокоррекции</td>
            <td>Ремонт блока управления</td>
            <td>Ремонт блока питания</td>
            <td>Ремонт кнопок управления</td>
            <td>Восстановление контактов платы</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 1400</td>
            <td>от 1600</td>
            <td>от 1100</td>
            <td>от 900</td>
            <td>от 200</td>
            <td>от 200</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">моноблоки</td>
          </tr>
          <tr>
            <td>Чистка от пыли</td>
            <td>Чистка от воды</td>
            <td>Сильно греется</td>
            <td>Глючит / Зависает</td>
            <td>Не включается</td>
            <td>Выключается</td>
          </tr>
          <tr>
            <td>от 900</td>
            <td>от 1250</td>
            <td>от 400</td>
            <td>от 350</td>
            <td>от 400</td>
            <td>от 800</td>
          </tr>
          <tr>
            <td>Разбит экран</td>
            <td>Нет изображения</td>
            <td>Отсутствует звук</td>
            <td>Сломан разъём</td>
            <td>Проблемы с ОС</td>
            <td>Удаление вирусов</td>
          </tr>
          <tr>
            <td>от 900</td>
            <td>от 500</td>
            <td>от 500</td>
            <td>от 600</td>
            <td>от 500</td>
            <td>от 350</td>
          </tr>
          <tr>
            <td>Не работает камера</td>
            <td>Не работает микрофон</td>
            <td>Не работает bluetooth</td>
            <td>Не работает модуль wi-fi</td>
            <td>Не видит устройства</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 450</td>
            <td>от 450</td>
            <td>от 500</td>
            <td>от 500</td>
            <td>от 350</td>
            <td>от 350</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">приставки</td>
          </tr>
          <tr>
            <td>Не читает диски</td>
            <td>Сильно греется</td>
            <td>Глючит / виснет</td>
            <td>Не загружается</td>
            <td>Не включается</td>
            <td>Выключается</td>
          </tr>
          <tr>
            <td>от 1450</td>
            <td>от 1500</td>
            <td>от 1000</td>
            <td>от 1350</td>
            <td>от 1350</td>
            <td>от 1350</td>
          </tr>
          <tr>
            <td>Присутствует рябь на экране</td>
            <td>Неиправен разъём HDMI</td>
            <td>Не работает wi-fi / bluetooth</td>
            <td>Нет системного накопителя</td>
            <td>Чистка от пыли и грязи</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 2450</td>
            <td>от 1350</td>
            <td>от 1000</td>
            <td>от 950</td>
            <td>от 800</td>
            <td>от 750</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">часы</td>
          </tr>
          <tr>
            <td>Замена дисплея</td>
            <td>Замена батареи</td>
            <td>Ремонт вибро</td>
            <td>Ремонт кнопки</td>
            <td>Чистка от воды</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 1500</td>
            <td>от 900</td>
            <td>от 500</td>
            <td>от 800</td>
            <td>от 1250</td>
            <td>от 500</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">стиралки</td>
          </tr>
          <tr>
            <td>Шумит/вибрирует</td>
            <td>Протекает</td>
            <td>Не крутится</td>
            <td>Не греет воду</td>
            <td>Не открывается</td>
            <td>Не включается</td>
          </tr>
          <tr>
            <td>от 700</td>
            <td>от 700</td>
            <td>от 1100</td>
            <td>от 900</td>
            <td>от 500</td>
            <td>от 500</td>
          </tr>
          <tr>
            <td>Не работает набор воды</td>
            <td>Не работает слив воды</td>
            <td>Не работает отжим вещей</td>
            <td>Не выставляется нужный режим</td>
            <td>Сломаны кнопки управления</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 800</td>
            <td>от 800</td>
            <td>от 900</td>
            <td>от 500</td>
            <td>от 500</td>
            <td>от 500</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">микроволновки</td>
          </tr>
          <tr>
            <td>Замена слюдяной пластины</td>
            <td>Замена предохранителя</td>
            <td>Замена трансформатора</td>
            <td>Замена мотора поддона</td>
            <td>Замена магнетрона</td>
            <td>Ремонт блока управления</td>
          </tr>
          <tr>
            <td>от 600</td>
            <td>от 400</td>
            <td>от 1100</td>
            <td>от 700</td>
            <td>от 900</td>
            <td>от 1400</td>
          </tr>
          <tr>
            <td>Замена сенсора</td>
            <td>Замена ТЭНа</td>
            <td>Ремонт дверцы</td>
            <td>Ремонт инвертора</td>
            <td>Ремонт проводки</td>
            <td>Другое</td>
          </tr>
          <tr>
            <td>от 900</td>
            <td>от 900</td>
            <td>от 900</td>
            <td>от 1400</td>
            <td>от 900</td>
            <td>от 400</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">холодильники</td>
          </tr>
          <tr>
            <td>Перемораживает</td>
            <td>Протекает</td>
            <td>Шумит</td>
            <td>Не морозит</td>
            <td>Не включается</td>
            <td>Выключается</td>
          </tr>
          <tr>
            <td>от 550</td>
            <td>от 700</td>
            <td>от 400</td>
            <td>от 550</td>
            <td>от 550</td>
            <td>от 750</td>
          </tr>
          <tr>
            <td>Не работает основная камера</td>
            <td>Не работает морозилка</td>
            <td>Покрывается льдом</td>
            <td>Холодильник бъет током</td>
            <td>Обнаружена утечка фриона</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 550</td>
            <td>от 450</td>
            <td>от 550</td>
            <td>от 400</td>
            <td></td>
            <td>от 350</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">посудомойки</td>
          </tr>
          <tr>
            <td>Шумит/вибрирует</td>
            <td>Не набирает воду</td>
            <td>Не греет воду</td>
            <td>Не включается</td>
            <td>Выключается</td>
            <td>Протекает</td>
          </tr>
          <tr>
            <td>от 700</td>
            <td>от 800</td>
            <td>от 900</td>
            <td>от 500</td>
            <td>от 900</td>
            <td>от 700</td>
          </tr>
          <tr>
            <td>Непрерывно заливает воду</td>
            <td>Не работает сушилка</td>
            <td>Не работает слив воды</td>
            <td>Плохо отмывает посуду</td>
            <td>Сломаны кнопки управления</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 800</td>
            <td>от 950</td>
            <td>от 800</td>
            <td>от 600</td>
            <td>от 500</td>
            <td>от 500</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">роботы пылесосы</td>
          </tr>
          <tr>
            <td>Не работает щетка</td>
            <td>Слабая тяга</td>
            <td>Не едет</td>
            <td>Не сканирует</td>
            <td>Не видит базу</td>
            <td>Проблемы с ПО</td>
          </tr>
          <tr>
            <td>от 610</td>
            <td>от 690</td>
            <td>от 500</td>
            <td>от 1100</td>
            <td>от 850</td>
            <td>от 610</td>
          </tr>
          <tr>
            <td>Не включается</td>
            <td>Не заряжается</td>
            <td>Выключается</td>
            <td>Громко гудит</td>
            <td>Пахнет гарью</td>
            <td>Другое</td>
          </tr>
          <tr>
            <td>от 450</td>
            <td>от 850</td>
            <td>от 800</td>
            <td>от 750</td>
            <td>от 750</td>
            <td>от 500</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">вспышки</td>
          </tr>
          <tr>
            <td>Попала влага</td>
            <td>Перегорела лампа</td>
            <td>Сломался башмак</td>
            <td>Повреждён корпус</td>
            <td>Перенёс падение</td>
            <td>Не включается</td>
          </tr>
          <tr>
            <td>от 1250</td>
            <td>от 1200</td>
            <td>от 1900</td>
            <td>от 850</td>
            <td>от 500</td>
            <td>от 1000</td>
          </tr>
          <tr>
            <td>Не реагирует на нажатия</td>
            <td>Не работает дисплей</td>
            <td>Сломан отражатель</td>
            <td>Отсутствует синхронизация</td>
            <td>Работает только на максимум</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 1000</td>
            <td>от 1800</td>
            <td>от 1500</td>
            <td></td>
            <td>от 1500</td>
            <td>от 500</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">объективы</td>
          </tr>
          <tr>
            <td>Пыль внутри объектива</td>
            <td>Царапины на линзе</td>
            <td>Не работает автофокус</td>
            <td>Дёргается изображение</td>
            <td>Заклинил светофильтр</td>
            <td>Повреждён байонет</td>
          </tr>
          <tr>
            <td>от 1000</td>
            <td>от 1000</td>
            <td>от 2500</td>
            <td>от 1500</td>
            <td>от 1000</td>
            <td>от 2000</td>
          </tr>
          <tr>
            <td>Чистка после жидкости</td>
            <td>Треснул корпус объектива</td>
            <td>Разбился светофильтр</td>
            <td>Люфт выдвижного тубуса</td>
            <td>Ремонт диафрагмы</td>
            <td>Ремонт кнопок управления</td>
          </tr>
          <tr>
            <td>от 1250</td>
            <td>от 850</td>
            <td>от 1000</td>
            <td>от 1500</td>
            <td>от 1500</td>
            <td>от 850</td>
          </tr>
          <tr>
            <td>Фотоаппарат не видит объектив</td>
            <td>Клинит после падения</td>
            <td>Издаёт трест (трещит)</td>
            <td>Промахи по фокусу</td>
            <td>Заклинило кольцо зума</td>
            <td>У меня другая неисправность</td>
          </tr>
          <tr>
            <td>от 1500</td>
            <td>от 1500</td>
            <td>от 1000</td>
            <td>от 2000</td>
            <td>от 2000</td>
            <td>от 500</td>
          </tr>
        </tbody>
      </table>
    </div>
    <!--  -->
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Внутриканальные наушники</td>
          </tr>
          <tr>
            <td>Замена внутренних кабелей наушников</td>
            <td>Замена дуги или динамиков</td>
            <td>Подбор аккумулятора и установка</td>
            <td>Ремонт USB наушников</td>
            <td>Ремонт беспроводных наушников</td>
            <td>Ремонт и замена гнезда наушников</td>
          </tr>
          <tr>
            <td>от 1000</td>
            <td>от 600</td>
            <td>от 1000</td>
            <td>от 700</td>
            <td>от 600</td>
            <td>от 500</td>
          </tr>
          <tr>
            <td>Ремонт и замена кабеля</td>
            <td>Ремонт и замена штекера</td>
            <td>Ремонт микрофона</td>
            <td>Другая неисправность</td>
            <td></td>
            <td></td>
          </tr>
          <tr>
            <td>от 500</td>
            <td>от 600</td>
            <td>от 600</td>
            <td>от 500</td>
            <td></td>
            <td></td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Полноразмерные наушники</td>
          </tr>
          <tr>
            <td>Замена внутренних кабелей наушников</td>
            <td>Замена дуги или динамиков</td>
            <td>Подбор аккумулятора и установка</td>
            <td>Ремонт USB наушников</td>
            <td>Ремонт беспроводных наушников</td>
            <td>Ремонт и замена гнезда наушников</td>
          </tr>
          <tr>
            <td>от 1000</td>
            <td>от 600</td>
            <td>от 1000</td>
            <td>от 700</td>
            <td>от 600</td>
            <td>от 500</td>
          </tr>
          <tr>
            <td>Ремонт и замена кабеля</td>
            <td>Ремонт и замена штекера</td>
            <td>Ремонт микрофона</td>
            <td>Другая неисправность</td>
            <td></td>
            <td></td>
          </tr>
          <tr>
            <td>от 500</td>
            <td>от 600</td>
            <td>от 600</td>
            <td>от 500</td>
            <td></td>
            <td></td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Колонки</td>
          </tr>
          <tr>
            <td>Замена аккумулятора</td>
            <td>Замена разъемов</td>
            <td>Не включается</td>
            <td>Не заряжается</td>
            <td>Не играет звук</td>
            <td>Не работают кнопки</td>
          </tr>
          <tr>
            <td>от 1000</td>
            <td>от 1000</td>
            <td>от 1000</td>
            <td>от 1300</td>
            <td>от 800</td>
            <td>от 300</td>
          </tr>
          <tr>
            <td>Ремонт корпуса</td>
            <td>Другая неисправность</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          </tr>
          <tr>
            <td>от 500</td>
            <td>от 300</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Гарнитура</td>
          </tr>
          <tr>
            <td>Замена внутренних кабелей наушников</td>
            <td>Замена дуги или динамиков</td>
            <td>Подбор аккумулятора и установка</td>
            <td>Ремонт USB наушников</td>
            <td>Ремонт беспроводных наушников</td>
            <td>Ремонт и замена гнезда наушников</td>
          </tr>
          <tr>
            <td>от 1000</td>
            <td>от 600</td>
            <td>от 1000</td>
            <td>от 700</td>
            <td>от 600</td>
            <td>от 500</td>
          </tr>
          <tr>
            <td>Ремонт и замена кабеля</td>
            <td>Ремонт и замена штекера</td>
            <td>Ремонт микрофона</td>
            <td>Другая неисправность</td>
            <td></td>
            <td></td>
          </tr>
          <tr>
            <td>от 500</td>
            <td>от 600</td>
            <td>от 600</td>
            <td>от 500</td>
            <td></td>
            <td></td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Фены</td>
          </tr>
          <tr>
            <td>Гул при работе</td>
            <td>Ремонт, замена нагревателя</td>
            <td>Не включается</td>
            <td>Пахнет гарью</td>
            <td>Сломаны лопасти</td>
            <td>Замена деталей корпуса</td>
          </tr>
          <tr>
            <td>от 430</td>
            <td>от 400</td>
            <td>от 350</td>
            <td>от 400</td>
            <td>от 400</td>
            <td>от 450</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Сушилки для рук</td>
          </tr>
          <tr>
            <td>Мелкий ремонт</td>
            <td>Средний ремонт</td>
            <td>Сложный ремонт</td>
            <td>Ремонт нагревателя</td>
            <td>Ремонт двигателя</td>
            <td>Другая неисправность</td>
          </tr>
          <tr>
            <td>от 1100</td>
            <td>от 1500</td>
            <td>от 2000</td>
            <td>от 2000</td>
            <td>от 2000</td>
            <td>от 800</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Освещение</td>
          </tr>
          <tr>
            <td>Лампа не включается</td>
            <td>Лампа сильно нагревается</td>
            <td>Замена запчастей</td>
            <td></td>
            <td></td>
            <td></td>
          </tr>
          <tr>
            <td>от 550</td>
            <td>от 400</td>
            <td>от 300</td>
            <td></td>
            <td></td>
            <td></td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Увлажнители</td>
          </tr>
          <tr>
            <td>Чистка, профилактика</td>
            <td>Ремонт/замена нагревателя</td>
            <td>Замена датчиков</td>
            <td>Не включается</td>
            <td>Гул при работе</td>
            <td>Не подает пар</td>
          </tr>
          <tr>
            <td>от 800</td>
            <td>от 800</td>
            <td>от 800</td>
            <td>от 600</td>
            <td>от 500</td>
            <td>от 700</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Пылесосы</td>
          </tr>
          <tr>
            <td>Не включается</td>
            <td>Не заряжается</td>
            <td>Выключается</td>
            <td>Громко гудит</td>
            <td>Пахнет гарью</td>
            <td>Слабая тяга</td>
          </tr>
          <tr>
            <td>от 450</td>
            <td>от 850</td>
            <td>от 800</td>
            <td>от 750</td>
            <td>от 750</td>
            <td>от 690</td>
          </tr>
          <tr>
            <td>Не подает воду</td>
            <td>Не собирает воду</td>
            <td>Не сматывается шнур</td>
            <td>Обрыв проводов</td>
            <td>Повреждения корпуса</td>
            <td>Другая неисправность</td>
          </tr>
          <tr>
            <td>от 410</td>
            <td>от 450</td>
            <td>от 790</td>
            <td>от 350</td>
            <td>от 600</td>
            <td>от 400</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Панели</td>
          </tr>
          <tr>
            <td>Не показывает</td>
            <td>Нет звука</td>
            <td>Нет сигнала</td>
            <td>Не включается</td>
            <td>Не видит устройства</td>
            <td>Не переключает каналы</td>
          </tr>
          <tr>
            <td>от 650</td>
            <td>от 495</td>
            <td>от 750</td>
            <td>от 750</td>
            <td>от 690</td>
            <td>от 500</td>
          </tr>
          <tr>
            <td>Со временем гаснет</td>
            <td>Замена динамика</td>
            <td>Замена подсветки</td>
            <td>Замена экрана</td>
            <td>Ремонт, замена блока питания</td>
            <td>Ремонт, замена материнской платы</td>
          </tr>
          <tr>
            <td>от 1000</td>
            <td>от 495</td>
            <td>от 500</td>
            <td>от 1900</td>
            <td>от 1000</td>
            <td>от 1100</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Электрические плиты</td>
          </tr>
          <tr>
            <td>Не включается подсветка</td>
            <td>Не работает духовка</td>
            <td>Не работает конфорка</td>
            <td>Не регулруется</td>
            <td>Не работает управление</td>
            <td>Другая неисправность</td>
          </tr>
          <tr>
            <td>от 950</td>
            <td>от 700</td>
            <td>от 800</td>
            <td>от 950</td>
            <td>от 950</td>
            <td>от 500</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Роутеры</td>
          </tr>
          <tr>
            <td>Замена прошивки</td>
            <td>Ремонт/ замена элементов питания</td>
            <td>Ремонт платы управления или платы Wi-Fi</td>
            <td>Не работает Ethernet</td>
            <td>Не опрелеяется</td>
            <td>Не включается</td>
          </tr>
          <tr>
            <td>600</td>
            <td>600</td>
            <td>1200</td>
            <td>от 400</td>
            <td>от 550</td>
            <td>от 450</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Духовые шкафы</td>
          </tr>
          <tr>
            <td>Вентилятор гонит холодный воздух</td>
            <td>Шумит или пищит</td>
            <td>Духовка слабо греет</td>
            <td>Духовой шкаф не нагревается</td>
            <td>Не работает совсем</td>
            <td>Посторонний запах</td>
          </tr>
          <tr>
            <td>от 1000</td>
            <td>от 600</td>
            <td>от 500</td>
            <td>от 500</td>
            <td>от 500</td>
            <td>от 700</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Варочные панели</td>
          </tr>
          <tr>
            <td>Замена конфорки</td>
            <td>Замена переключателя</td>
            <td>Замена ручки</td>
            <td>Замена электронного модуля</td>
            <td>Отключается</td>
            <td>Не включается</td>
          </tr>
          <tr>
            <td>от 1200</td>
            <td>от 920</td>
            <td>от 800</td>
            <td>от 1200</td>
            <td>от 900</td>
            <td>от 900</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Книги электронные</td>
          </tr>
          <tr>
            <td>Разбито стекло (тачскрин)</td>
            <td>Разбит экран (дисплей)</td>
            <td>Не работает модуль wi-fi</td>
            <td>Не видит карту памяти</td>
            <td>Не работает кнопка включения</td>
            <td>Чистка от воды</td>
          </tr>
          <tr>
            <td>от 750</td>
            <td>от 1400</td>
            <td>от 400</td>
            <td>от 650</td>
            <td>от 700</td>
            <td>от 900</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-6 col-lg-4 mb-30">
      <table
        class="el-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
      >
        <tbody>
          <tr>
            <td colspan="6">Квадрокоптеры</td>
          </tr>
          <tr>
            <td>Замена шестерней</td>
            <td>Замена лопастей</td>
            <td>Замена одного мотора</td>
            <td>Замена материнской платы</td>
            <td>Замена платы навигации</td>
            <td>Замена крестовины</td>
          </tr>
          <tr>
            <td>от 1900</td>
            <td>от 990</td>
            <td>от 2500</td>
            <td>от 2500</td>
            <td>от 990</td>
            <td>от 1800</td>
          </tr>
          <tr>
            <td>Замена корпуса с камерой</td>
            <td>Замена контроллера</td>
            <td>Замена посадочных шасси</td>
            <td>Замена GPS модуля</td>
            <td>Замена GPS модуля (б/у)</td>
            <td>Замена мотора квадрокоптера</td>
          </tr>
          <tr>
            <td>от 1400</td>
            <td>от 1900</td>
            <td>от 1400</td>
            <td>от 1400</td>
            <td>от 1400</td>
            <td>от 1400</td>
          </tr>
          <tr>
            <td>Замена шлейфа подвеса</td>
            <td>Восстановления после попадания воды</td>
            <td>Прошивка (обновление, откат версии)</td>
            <td>Замена разъема питания</td>
            <td>Тянет в сторону</td>
            <td>Нет тяги</td>
          </tr>
          <tr>
            <td>от 1400</td>
            <td>от 1400</td>
            <td>от 1400</td>
            <td>от 1400</td>
            <td>от 400</td>
            <td>от 1400</td>
          </tr>
          <tr>
            <td>Не взлетает</td>
            <td>Не включается</td>
            <td>Не крутится винт</td>
            <td>Не работает один винт</td>
            <td>Не реагирует на пульт</td>
            <td>Другая неиспрасность</td>
          </tr>
          <tr>
            <td>от 450</td>
            <td>от 1100</td>
            <td>от 1100</td>
            <td>от 400</td>
            <td>от 900</td>
            <td>от 500</td>
          </tr>
        </tbody>
      </table>
    </div>
  </div>
</template>

<script>
export default {
  name: 'MalfunctionPanel',

  data() {
    return {
      loading: false
    }
  },

  computed: {
    malfunctions() {
      return this.$store.state.repair.malfunction.malfunctions
    }
  },

  methods: {
    openModalCreate() {
      this.$store.commit('settings/SWITCH_DRAWNER', {
        dranwer: 'windowCreateMalfunction',
        status: true
      })
    },

    openModalUpdate(collectionItem) {
      this.$store.commit('repair/malfunction/SET_MALFUNCTION', collectionItem)
      this.$store.commit('settings/SWITCH_DRAWNER', {
        dranwer: 'windowUpdateMalfunction',
        status: true
      })
    },

    /**
     * -
     */
    async removeMalfuntion(collectionId) {
      this.loading = true
      try {
        await this.$axios.$delete(
          '/api/v1/repair/malfunction/remove/' + collectionId
        )
        this.$store.dispatch('repair/malfunction/fetchItems')
        await this.$notify({
          message: 'Коллекция успушно удалена!',
          customClass: 'success-notyfy'
        })
      } catch (e) {
        this.$store.commit('SET_ERROR', e.response.data.message)
        console.error('Неудалосьудалить коллекцию неисправностей', e)
      } finally {
        this.loading = false
      }
    }
  }
}
</script>

<style scoped>
tbody td {
  text-align: center;
  font-size: 12px;
}
td[colspan='5'] {
  font-size: 20px;
}
</style>
