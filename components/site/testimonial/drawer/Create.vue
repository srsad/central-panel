<template>
  <el-drawer
    :visible.sync="$store.state.settings.drawerSiteTestimonialCreate"
    :with-header="false"
  ...
  >
    <el-form
      ...
      class="row m-15"
    >
      <div class="col-12"><h3 class="mb-20">Добавить отзыв</h3></div>
      <!-- site_url -->
  ...
  methods: {
    ...
    async onCreate() {
      ...
      try {
        ...
        await this.$store.dispatch('site/testimonial/create', formData)
        this.clearForm()
        this.$notify({
        ...
        if (formData.site_url === this.$store.state.site.testimonial.siteUrl) {
          this.$store.dispatch(
