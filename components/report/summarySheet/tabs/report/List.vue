...
export default {
  ...
  methods: {
    ...
    async removeTable() {
      this.loading = true
      try {
        await this.$axios.$delete('/api/v1/report/summory/remove/' + this.tableData._id)
        this.$notify({
    ...
    async updateTable() {
      ...
      for await (const item of this.tableData.brands) {
        ...
        const commonExpenses = +item.common_expenses.balance + item.common_expenses.pk + item.common_expenses.seo + item.common_expenses.common
        // заявки
