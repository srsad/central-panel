<template>
  ...
  >
    ...
    >
      ...
      <div class="col-6">
        <!-- rservice -->
        <el-form-item prop="rservice" label="Цена ремонта">
          <el-input v-model="form.rservice" placeholder="Цена ремонта" />
        </el-form-item>
        <!-- end rservice -->
      </div>
      <!-- description -->
      ...
      <div v-if="form.excepts.length > 0" class="col-12">
        ...
        >
          ...
          <div class="col-7 mb-10">
            ...
            >
              <el-option
                v-for="(el, _idx) in $store.state.repair.device.devices"
                :key="_idx"
                :label="el.name"
                :value="el.name"
              />
...
export default {
  ...
  methods: {
    ...
    async chekForm() {
      this.loading = true
      try {
        this.form.brand = this.$store.state.repair.brand.brand.name.toLowerCase()
        this.form.category = this.$store.state.repair.category.selectCategory.name.toLowerCase()
